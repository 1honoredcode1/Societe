...
const Messages = () => {
  ...
  const filteredChats = useMemo(() => {
    ...
    return chats.filter((chat) => {
      ...
      return (
        chat.listing?.title?.toLocaleLowerCase().includes(query) ||
        chatUser?.name?.toLocaleLowerCase().includes(query)
      );
    });
  }, [chats, searchQuery]);
  ...
  return (
    <div className="mx-auto min-h-screen px-6 md:px-16 lg:px-24 xl:px-32">
      <div className="py-10">
        ...
        <p className="text-gray-600">Chat with people</p>
      </div>
      {/* Search Bar */}
      <div className="relative max-w-xl mb-8">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-primary"
        />
      </div>
      {/* Chats */}
      {loading ? (
        ...
        <div className="bg-white rounded-lg shadow-xs border border-gray-200 p-16 text-center">
          ...
          <p className="text-gray-600">
            {searchQuery
              ...
              : "Start a coversation with a person first"}{" "}
          </p>
        </div>
      ) : (
